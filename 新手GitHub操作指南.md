...
**💡 隐私说明**：
- 项目已配置为**只上传代码**，不上传音频文件
- 你的音乐文件、封面图片、歌词文件都**保留在本地**，不会上传到GitHub
- GitHub上只有代码框架，没有任何隐私内容
...
由于隐私保护，音频文件已被配置为**不上传到GitHub**：
- ✅ 你的音乐文件、歌词、封面图片**安全保留在本地**
- ❌ GitHub Actions构建的APK**不包含音频文件**
- 📱 安装后的应用只有界面框架，没有音乐内容

**如何获得包含音频的APK？**
有两个方案：
1. **本地构建**（推荐）：需要安装Android SDK，可以构建包含你本地音频的APK
2. **手动添加**：将音频文件通过其他方式传到手机后，应用可以读取（需要修改代码支持外部存储）
